...
def MB():
    """Root to the html form of the add of a new beer"""
    return render_template('modif_bar_form.html')

@app.route('/modifbar', methods=['GET','PUT'])
def modifbar():
    if request.method == 'GET':
        nameb = request.args.get('beer')
        data = {}
        data = Bars.find({"Nom":name}, {"_id":0})
        return returning(data)
        #return render_template('modif_bar_form2.html')
        ...
        Bars.update({"Nom":nameb}, {'$set': {"Nom":name, "Adresse":adresse, "Pays":country, "Horaires":schedule, "Bières":beers, "Bieres Pressions":pbeers, "Bieres Bouteilles":botbeers, "Loc": [lng,lat]}})
        data = Bars.find({"Nom":name}, {"_id":0})
        return returning(data)
...
def newbar():
    ...
    if request.method == 'POST':
        ...
        test = Bars.find({"Nom":name, "Adresse":adresse, "Pays":country}, {"_id":0})
        json2 = dumps(test)
        if  json2 != "[]":
            return redirect(url_for('fail_new_bar', name=name))
        else:
            Bars.insert({"Nom":name, "Adresse":adresse, "Pays":country, "Horaires":schedule, "Bières":beers, "Bieres Pressions":pbeers, "Bieres Bouteilles":botbeers, "Loc": [lng,lat]})
...
def newbeer():
    ...
    if request.method == 'POST':
        ...
        # Test of presence of the resource in the collection
        # Test de présence dans la collection
        test = Beers.find({"Nom":user, "Goût":tast, "Arômes":flavour, "Alcool":alcool, "Brasserie":brasserie, "Type":tipe}, {"_id":0})
        json2 = dumps(test)
        if  json2 != "[]":
...
def newbrass():
    ...
    if request.method == 'POST':
        ...
        # Test of presence of the resource in the collection
        # Test de présence dans la collection
        test = Brass.find({"Nom":name, "Adresse":adresse, "Pays":country}, {"_id":0})
        json2 = dumps(test)
        if  json2 != "[]":
            return redirect(url_for('fail_new_brass', name=name))
        else:
            Brass.insert({"Nom":name, "Adresse":adresse, "Pays":country, "Bières":beers, "Histoire":past, "Loc": [lng,lat]})
            return redirect(url_for('success_new_bar', name=name))
    else:
